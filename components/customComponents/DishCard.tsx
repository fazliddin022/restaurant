...
const DishCard = ({ id, name, description, price, img }: DishCardProps) => {
  ...
  return (
    <Link href={`/menu/${id}`}>
      <div className="w-[263px] relative min-h-[310px] bg-white/40 rounded-[38px] flex flex-col items-center hover:bg-white/60 transition-colors duration-300 cursor-pointer">
        <Image
          src={img}
          alt={name}
          width={224}
        ...
        <div className="w-full px-5 h-full flex flex-col justify-between pb-4">
          <div>
            <div className="w-full flex justify-between items-center">
              ...
              <Button size="icon" className="cursor-pointer bg-transparent mt-0.5">
                <HeartIcon />
              </Button>
            </div>
